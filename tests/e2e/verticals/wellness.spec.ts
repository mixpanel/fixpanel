...
test.describe('ourHeart', () => {
  ...
  test('Homepage loads with tracking', async ({ page }) => {
    ...
    const hasStart = consoleTracker.hasLog('START wellness');
  ...
  test('Navigation tracks page views', async ({ page }) => {
    ...
    const links = await page.locator('a[href*="/wellness"]').all();
  ...
  test('Reset works on wellness', async ({ page }) => {
    await page.goto('/wellness');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
  ...
  test('Unique session tracking', async ({ page }) => {
    ...
    console.log('🎯 Session:');
    console.log('  - START wellness:', hasStart ? '✅' : '❌');
