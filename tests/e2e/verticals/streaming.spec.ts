...
test.describe('meTube', () => {
  ...
  test('Homepage loads with tracking', async ({ page }) => {
    ...
    const hasStart = consoleTracker.hasLog('START streaming');
  ...
  test.skip('Form interactions tracked', async ({ page }) => {
    await page.goto('/streaming');
    await page.waitForLoadState('networkidle');
  ...
  test('Reset works on streaming', async ({ page }) => {
    await page.goto('/streaming');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(1500);
  ...
  test('Unique session tracking', async ({ page }) => {
    ...
    console.log('🎯 Session:');
    console.log('  - START streaming:', hasStart ? '✅' : '❌');
